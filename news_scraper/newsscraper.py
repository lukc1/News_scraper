...
class NewsScraper:
    def __init__(self):
        ...
        self.session.headers.update({"User-Agent": "NewsScraper/1.0"})

    def getdata(self, url):
        response = self.session.get(url, timeout=10)
        return BeautifulSoup(response.text, "html.parser")

    def fetch_articles(self, links, select_title, select_content, limit):
        data = load()
    ...
    def risingnepal(self, limit, url="https://risingnepaldaily.com/"):
        ...
        for cat in list(dict.fromkeys(cats)):
            cat_soup = self.getdata(cat)
            for a in cat_soup.select("a[href*='/news/']"):
                href = str(a.get("href"))
                links.append(href)

        self.fetch_articles(links, "h1", "div.blog-details p", limit)

    def kathmandupost(self, limit, url="https://kathmandupost.com/"):
        soup = self.getdata(url)
        if not soup:
            return
        links = []
        ...
        self.fetch_articles(links, "title", "p", limit)

    def himalayantimes(self, limit, url="https://thehimalayantimes.com/"):
        soup = self.getdata(url)
        if not soup:
            return
        cats = []
        ...
        self.fetch_articles(
            links, "h1", "div.ht-article-details article div.post-content p", limit
        )
...
if __name__ == "__main__":
    ...
    scrapers = [
        scraper.risingnepal,
        scraper.kathmandupost,
        scraper.himalayantimes,
    ]
    limit_per_site = 10
    for s in scrapers:
